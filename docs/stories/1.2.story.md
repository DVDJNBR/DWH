...
- [ ] **Analyze existing `terraform/modules/stream_analytics/main.tf`**: Understand current `dim_vendor` processing and identify necessary changes for SCD Type 2.
- [ ] **Modify `terraform/modules/stream_analytics/main.tf`**: Update the Stream Analytics query to implement SCD Type 2 logic for the `dim_vendor` dimension (e.g., adding `start_date`, `end_date`, `is_current` columns and logic). (AC: 1)
- [ ] **Verify `make deploy`**: Ensure the Terraform deployment of the Stream Analytics job still functions correctly with the updated query.
